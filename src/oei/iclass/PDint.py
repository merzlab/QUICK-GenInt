...
class PDint(OEint):
    ...
    # generate code to save [p|d] integral gradients
    def save_int_grad(self):
        ...
        self.fhga.write("    DDint_0 dd(PAx, PAy, PAz, PBx, PBy, PBz, PCx, PCy, PCz, Zeta, YVerticalTemp); \n")
        self.fhga.write("    PFint_0 pf(PAx, PAy, PAz, PBx, PBy, PBz, PCx, PCy, PCz, Zeta, YVerticalTemp); \n\n")

        for i in range(0,6):
            self.fhga.write("    LOC2(store2, %d, %d, STOREDIM, STOREDIM) = sd.x_%d_%d;\n" % (0, i+4, 0, i+4))

        for i in range(0,3):
            for j in range(0,3):
                self.fhga.write("    LOC2(store2, %d, %d, STOREDIM, STOREDIM) = pp.x_%d_%d;\n" % (i+1, j+1, i+1, j+1))

        for i in range(0,6):
            for j in range(0,6):
                self.fhga.write("    LOC2(store2, %d, %d, STOREDIM, STOREDIM) = dd.x_%d_%d;\n" % (i+4, j+4, i+4, j+4))

        for i in range(0,10):
            for j in range(0,3):
                self.fhga.write("    LOC2(store2, %d, %d, STOREDIM, STOREDIM) = pf.x_%d_%d;\n" % (j+1, i+10, j+1, i+10)) 
